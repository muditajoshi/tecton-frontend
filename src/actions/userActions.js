...
export const refreshLogin = (email) => async (dispatch, getState) => {
	try {
		...
		if (userInfo.isSocialLogin) {
			...
			const { data } = await axios.post(
				`${process.env.REACT_APP_PROXY_URL}/api/users/refresh`,
				{
					email,
...
export const registerUser = (firstName,lastName,gender, email, password) => async (dispatch) => {
	try {
...
export const sendVerficationEmail = (email) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post(
			`${process.env.REACT_APP_PROXY_URL}/api/users/confirm`,
			{ email },
			config
		);
	...
	async (dispatch, getState) => {
		try {
			...
			const { data } = await axios.get(
				`${process.env.REACT_APP_PROXY_URL}/api/users/confirm/${emailToken}`
			);
	...
	try {
		...
		if (userInfo.isSocialLogin) {
			...
			let { data } = await axios.post(
				`${process.env.REACT_APP_PROXY_URL}/api/users/passport/data/`,
				{ id },
				config
			);
			dispatch({
			...
			const { data } = await axios.get(`${process.env.REACT_APP_PROXY_URL}/api/users/${id}`, config);
			console.log(data)
			dispatch({
				type: USER_DETAILS_SUCCESS,
				payload: { ...data, isSocialLogin: false },
...
export const listAllUsers =
	...
	async (dispatch, getState) => {
		try {
			...
			const { data } = await axios.get(
				`${process.env.REACT_APP_PROXY_URL}/api/users?pageNumber=${pageNumber}`,
				config
			);
...
export const deleteUser = (id) => async (dispatch, getState) => {
	try {
		...
		await axios.delete(`${process.env.REACT_APP_PROXY_URL}/api/users/${id}`, config);
...
export const updateUser = (user) => async (dispatch, getState) => {
	try {
		...
		const isSocial = userInfo.isSocialLogin;
		const { data } = await axios.put(
			`${process.env.REACT_APP_PROXY_URL}/api/users/${user._id}`,
			user,
			config
		);
