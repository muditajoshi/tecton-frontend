...
export const getOrderDetails = (orderID) => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.get(`${process.env.REACT_APP_PROXY_URL}/api/orders/${orderID}`, config);
...
export const deliverOrder = (orderID) => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.put(
			`${process.env.REACT_APP_PROXY_URL}/api/orders/${orderID}/deliver`,
			{},
			config
		);
	...
	async (dispatch, getState) => {
		try {
			...
			const { data } = await axios.get(
				`${process.env.REACT_APP_PROXY_URL}/api/orders?pageNumber=${pageNumber}`,
				config
			);
