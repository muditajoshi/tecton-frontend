...
const NewsEditPage = ({history,match}) => {
  ...
  const config = userInfo?.isSocialLogin
    ...
    : {
      ...
      useEffect(()=>{
        axios.get(`${process.env.REACT_APP_PROXY_URL}/api/news/get-news-by-id/${match.params.id}`,config).then((res)=>{
            console.log(res.data.news);
            setImage(res.data.news.img);
            setDescription(res.data.news.newsDescription);
            setLink(res.data.news.newsLink)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // console.log(image)  
    axios.put(`${process.env.REACT_APP_PROXY_URL}/api/news/update-news/${match?.params?.id}`,{
  ...
  return (
    <div>
      ...
      <FormContainer style={{ marginTop: "-2em" }}>
        <h1 className="productEditPage_heading"> Edit News Section</h1>
